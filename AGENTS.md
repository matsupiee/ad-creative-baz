...
1 セッション = 1 タスク。コンテキストウィンドウの枯渇を避けるため、複数タスクを1セッションで実装しない。

```
1. Orient   → progress.md と git log を読む
2. Plan     → /plan で Planner に設計させる（変更範囲・影響ファイル・検証方法）
3. Execute  → Plan に沿って実装。1ファイルずつ、小さく commit
4. Verify   → /verify で型チェック・lint・振る舞い確認
5. Review   → /review で自己レビュー。指摘はループで解消
6. Record   → progress.md を更新して commit
```
...
- **破壊的操作は確認必須**: `db:push`, migrations の巻き戻し, `rm -rf`, `git reset --hard`, `git push --force` は必ず人間に確認。
