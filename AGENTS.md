...
- **`main` 直 commit / push 禁止**: 1 タスク = 1 feature branch + 1 PR。branch 名は `feat/`, `fix/`, `chore/`, `docs/` プレフィックス。マージは squash + branch 削除。詳細は [`docs/agents/workflow.md`](docs/agents/workflow.md) §7。**`git push` / `gh pr create` / `gh pr merge --squash --delete-branch` は恒久的に事前承認済み**：verify が通ったら**確認を求めず黙って push → PR → squash merge まで一気に実行する**（リポジトリオーナーは個人 1 名、CI もまだ無い運用前提）。「マージしていいですか？」「PR 化しますか？」の類いを聞くのは禁止。
